...
class Endpoints {
  ...
  static forgotPassword = async ({ email }: { email: string }) => {
    const response = await base.post(`/account/forgot_password`, {
      email: email,
    });
    return response?.data;
  };
  ...
  static resetPassword = async ({
    ...
    const response = await base.post(`/account/reset_password/${otp}`, {
      password: password,
    });
    return response?.data;
  };
  ...
  static checkFavourite = async ({ id, jwt }: { id: string; jwt?: string }) => {
    const headers = {
      Authorization: `Bearer ${jwt}`,
    };
    try {
