  ...
  return (
    <section className="flex flex-col justify-center items-center">
      ...
      <div className="flex items-center justify-center flex-wrap gap-x-7 gap-y-7">
        {styles.map((item) => {
          return <div key={item.displayName} className="bg-[var(--lightish-grey)] rounded-xl boxshadow w-72 h-96 flex-col flex items-start justify-start">
            <div className="text-[var(--onDark)] my-5 mx-4 text-base">{item.displayName}</div>
            <img src={item.imgsrc} alt={item.displayName} className="w-full h-36 object-cover "/>
            <div className="my-5 mx-4 text-sm">{item.descript}</div>
            <div className="w-full flex mt-auto items-center justify-end">
              <div className="rounded-full h-10 w-24 border-2 border-[var(--onDark)] flex items-center justify-center my-5 mx-4 cursor-pointer hover:bg-[var(--onDark)] text-[var(--onDark)] hover:text-[var(--lightish-grey)] hover:-translate-y-1 duration-300">Explore</div>
            </div>
